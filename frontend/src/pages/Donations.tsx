            ...
            {donations.map((donation) => (
              <tr key={donation.id} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4 text-sm">
                  <button onClick={() => openModal(donation)} className="text-blue-600 hover:text-blue-800 ml-2">
                    <Plus size={16} />
                  </button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-xl font-bold mb-4">{editingDonation ? 'تعديل تبرع' : 'إضافة تبرع'}</h2>
            <form onSubmit={(e) => {
              e.preventDefault()
